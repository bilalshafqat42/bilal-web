/**
 * Search ranking, with no data imports.
 *
 * Split out of `searchIndex.ts` on 2026-09-16. That file imports `pillars.ts`,
 * `serviceDepth.ts`, `caseStudies.ts`, `faqs.ts` and `disciplines.ts` to build
 * the index, and it was imported by two client components — so every one of
 * those data files was compiled into the browser bundle and downloaded on all
 * 28 routes, including the privacy page.
 *
 * Everything here is pure: a query in, ranked chunks out. It carries no site
 * content, so a client component can import it for a few kilobytes. The index
 * itself now arrives over the network from `/search-index.json`, fetched only
 * when someone actually opens a search panel.
 *
 * The tuned constants below are covered by `npm run search-check`, which runs
 * 32 fixtures against them. Do not adjust one without running it.
 */
